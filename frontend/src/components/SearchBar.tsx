...
export const SearchBar = ({ value, onChange, onSearch, placeholder = "Search peptides, research chemicals..." }: SearchBarProps) => {
  ...
  const generateSuggestions = (searchValue: string) => {
    ...
    peptidesData.forEach(peptide => {
      // Add peptide names (including aliases in parentheses)
      if (peptide.name.toLowerCase().includes(searchLower)) {
        suggestions.add(peptide.name);
      }
      
      // Extract aliases from names (e.g., "PT-141 (Bremelanotide)" -> add both)
      const aliasMatch = peptide.name.match(/^(.+?)\s*\((.+?)\)$/);
      if (aliasMatch) {
        const mainName = aliasMatch[1].trim();
        const alias = aliasMatch[2].trim();
        
        if (mainName.toLowerCase().includes(searchLower) || alias.toLowerCase().includes(searchLower)) {
          suggestions.add(peptide.name);
        }
      }
      ...
      // Add relevant description keywords (improved)
      const descriptionWords = peptide.fullDescription.toLowerCase()
        .replace(/[^\w\s]/g, ' ')
        .split(/\s+/)
      ...
      peptide.useCases.forEach(useCase => {
        if (useCase.toLowerCase().includes(searchLower)) {
          suggestions.add(useCase);
        }
      });
      
      peptide.benefits.forEach(benefit => {
        if (benefit.toLowerCase().includes(searchLower)) {
